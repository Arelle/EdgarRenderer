  ...
  filterUpdates: function () {
    ...
    Taxonomies.loadingTaxonomyCount(function () {
      ...
      Taxonomies.updateTaxonomyCount(
        Object.keys(UserFiltersState.getUserSearch).length === 2
      );
      var endPerformance = performance.now();
      console.debug(
        "UserFiltersState.filterUpdates() completed in: " +
    ...
    if (UserFiltersState.getDataRadios && enabledTaxonomy) {
      switch (UserFiltersState.getDataRadios) {
        case 5: {
          ...
          if (
            current.hasAttribute("isAdditionalItemsOnly") &&
        ...
        case 4: {
          ...
          if (!current.hasAttribute("isNegativesOnly")) {
            current.setAttribute(
              "isNegativesOnly",
              current.getAttribute("sign") === "-" ? true : false
            );
          }
          if (
            current.hasAttribute("isNegativesOnly") &&
            current.getAttribute("isNegativesOnly") === "true"
          ) {
            return true;
          }
          return false;
        ...
        case 3: {
          ...
          if (!current.hasAttribute("isCalculationsOnly")) {
            ...
            if (
              ...
              if (
                FiltersContextref.getDimensions(
                  current.getAttribute("contextRef")
                ).length === 0
              ) {
            ...
            current.setAttribute(
              "isCalculationsOnly",
              elementIsCalculationsOnly
            );
          }
          if (
            current.hasAttribute("isCalculationsOnly") &&
            current.getAttribute("isCalculationsOnly") === "true"
          ) {
            return true;
          }
          return false;
        ...
        case 2: {
          ...
          if (!current.hasAttribute("isTextOnly")) {
            current.setAttribute(
              "isTextOnly",
              current["tagName"].split(":")[1].toLowerCase() === "nonnumeric"
                ? true
                : false
            );
          }
          if (
            current.hasAttribute("isTextOnly") &&
            current.getAttribute("isTextOnly") === "true"
          ) {
            return true;
          }
          return false;
        ...
        case 1: {
          ...
          if (!current.hasAttribute("isAmountsOnly")) {
            current.setAttribute(
              "isAmountsOnly",
              current["tagName"].split(":")[1].toLowerCase() === "nonfraction"
                ? true
                : false
            );
          }
          if (
            current.hasAttribute("isAmountsOnly") &&
            current.getAttribute("isAmountsOnly") === "true"
          ) {
            return true;
          }
          return false;
      ...
      switch (UserFiltersState.getTagsRadios) {
        case 2: {
          ...
          if (!current.hasAttribute("isCustomOnly")) {
            current.setAttribute(
              "isCustomOnly",
              current.getAttribute("name").split(":")[0].toLowerCase() ===
          ...
          if (
            current.hasAttribute("isCustomOnly") &&
            current.getAttribute("isCustomOnly") === "true"
          ) {
            return true;
          }
          return false;
        ...
        case 1: {
          ...
          if (
            current.hasAttribute("isStandardOnly") &&
            current.getAttribute("isStandardOnly") === "true"
          ) {
            return true;
          }
          return false;
  ...
  periods: function (current, enabledTaxonomy) {
    if (UserFiltersState.getPeriod.length && enabledTaxonomy) {
      for (var i = 0; i < UserFiltersState.getPeriod.length; i++) {
        if (
          UserFiltersState.getPeriod[i]["contextRef"].indexOf(
            current.getAttribute("contextRef")
          ) >= 0
        ) {
          return true;
    ...
    if (UserFiltersState.getMeasure.length && enabledTaxonomy) {
      if (current.hasAttribute("unitref")) {
        for (var i = 0; i < UserFiltersState.getMeasure.length; i++) {
          if (
            current.getAttribute("unitref") === UserFiltersState.getMeasure[i]
          ) {
            return true;
          }
        }
  ...
  members: function (current, enabledTaxonomy) {
    if (UserFiltersState.getMembers.length && enabledTaxonomy) {
      for (var i = 0; i < UserFiltersState.getMembers.length; i++) {
        for (
          var k = 0;
          k < UserFiltersState.getMembers[i]["parentID"].length;
          k++
        ) {
          if (
            current.getAttribute("contextRef") ===
            UserFiltersState.getMembers[i]["parentID"][k]
          ) {
            return true;
          }
        }
    ...
    if (UserFiltersState.getType.length && enabledTaxonomy) {
      for (var i = 0; i < UserFiltersState.getType.length; i++) {
        if (
          ...
          if (
            UserFiltersState.getType[i].toLowerCase() ===
            current.getAttribute("name").split(":")[0].toLowerCase()
          ) {
            return true;
          }
  ...
  balances: function (current, enabledTaxonomy) {
    if (UserFiltersState.getBalance.length && enabledTaxonomy) {
      ...
      if (tagInformation.length && tagInformation[0]["crdr"]) {
        if (
          UserFiltersState.getBalance.indexOf(tagInformation[0]["crdr"]) >= 0
        ) {
          return true;
        }
      }
    ...
    if (Object.keys(UserFiltersState.getUserSearch).length === 2) {
      if (UserFiltersState.getUserSearch["options"].indexOf(1) >= 0) {
        ...
        fullContentToRegexAgainst +=
          " " + SearchFunctions.elementNameForRegex(current);
      }

      if (UserFiltersState.getUserSearch["options"].indexOf(2) >= 0) {
        // include fact content
        fullContentToRegexAgainst +=
          " " + SearchFunctions.elementContentForRegex(current);
      }

      if (UserFiltersState.getUserSearch["options"].indexOf(3) >= 0) {
        // include labels
        fullContentToRegexAgainst +=
          " " + SearchFunctions.elementLabelForRegex(current);
      }

      if (UserFiltersState.getUserSearch["options"].indexOf(4) >= 0) {
        // include definitions
        fullContentToRegexAgainst +=
          " " + SearchFunctions.elementDefinitionForRegex(current);
      }

      if (UserFiltersState.getUserSearch["options"].indexOf(5) >= 0) {
        // include dimensions
        fullContentToRegexAgainst +=
          " " + SearchFunctions.elementDimensionsForRegex(current);
      }

      if (
        UserFiltersState.getUserSearch["options"].indexOf(6) >= 0 ||
        ...
        fullContentToRegexAgainst +=
          " " +
          SearchFunctions.elementReferencesForRegex(
            current,
            UserFiltersState.getUserSearch
          );
      }
      highlight = UserFiltersState.getUserSearch.regex.test(
        fullContentToRegexAgainst
      );
    }
  ...
  setContinuedAtHighlight: function (current, highlight) {
    if (current) {
      ...
      if (current.hasAttribute("continuedat")) {
        UserFiltersState.setContinuedAtHighlight(
          document
            .getElementById("dynamic-xbrl-form")
            .querySelector(
              '[id="' + current.getAttribute("continuedat") + '"]'
            ),
          highlight
        );
      }
