...
export const ModalsNested = {
	...
	createLabelCarousel: () => {
		// reset 
		document.querySelector('#modal-fact-nested-label-carousel')!.innerHTML = '';
		...
		document.getElementById('modal-fact-nested-label-carousel')
			?.querySelector('.carousel-item')?.classList
			.add('active');
	},
	...
	nestedClickEvent: (event: MouseEvent | KeyboardEvent, element: HTMLElement) => {
		if (
