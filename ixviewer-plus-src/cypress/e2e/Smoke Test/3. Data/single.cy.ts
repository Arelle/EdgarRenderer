...
describe(`3. Data => ${filing.name} - ${filing.form}`, () => {
    beforeEach(() => {
    ...
    it('should show menu', () => {
        cy.get(`#nav-filter-data`).click();
        cy.get(`#nav-filter-data + .dropdown-menu`).should(`be.visible`);
    });

    it('should hide menu', () => {
        cy.get(`#nav-filter-data`).click();
        cy.get(`#nav-filter-data + .dropdown-menu`).not(`be.visible`);
    });
});
