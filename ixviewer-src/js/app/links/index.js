...
var Links = {
  init: function () {
    if (
      Constants.getMetaSourceDocuments.length > 1 &&
      Constants.getMetaVersion >= 2.1
    ) {
      Links.updateLinks();
    }
  ...
  absoluteLinks: function () {
    var foundLinks = document
      .getElementById("dynamic-xbrl-form")
      .querySelectorAll("[data-link],[href]");
    ...
    Constants.getMetaSourceDocuments.forEach(function (current) {
      ...
      if (current !== HelpersUrl.getHTMLFileName) {
        link.addEventListener("click", function (e) {
          Links.clickEventInternal(e, link);
        });
