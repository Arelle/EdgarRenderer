...
var UserFiltersMoreFiltersPeriod = {
  clickEvent: function (event, element) {
    if (event["target"] && event["target"].getAttribute("data-filter")) {
      var dataFilter = JSON.parse(
        decodeURIComponent(event["target"].getAttribute("data-filter"))
      );
      if (dataFilter["all"]) {
  ...
  childClick: function (event, element, parentIndex, childIndex) {
    UserFiltersMoreFiltersPeriod.checkToggle(
      event,
      element,
      parentIndex,
      childIndex
  ...
  checkToggleAll: function (filter, element, parentIndex) {
    var foundInputs = document.querySelectorAll(
      "#period-filters-accordion-" + parentIndex + " input[type=checkbox]"
    );
  ...
  checkToggle: function (filter, element, parentIndex, childIndex) {
    if (element.checked) {
      // check this child
      UserFiltersMoreFiltersPeriod.setStateFromChild(
        parentIndex,
        childIndex,
        true
      );
  ...
  setStateFromParent: function (parentIndex, addIsTrueRemoveIsFalse) {
    if (addIsTrueRemoveIsFalse) {
      // we add to our state
      var newPeriods = UserFiltersMoreFiltersPeriodSetUp.periodsOptions[
        parentIndex
      ...
      UserFiltersState.getPeriod =
        UserFiltersState.getPeriod.concat(newPeriods);
    } else {
      // we remove from our state
      var removePeriods = UserFiltersMoreFiltersPeriodSetUp.periodsOptions[
        parentIndex
      ]["options"].map(function (current, index) {
        if (UserFiltersState.getPeriod.indexOf(current) >= 0) {
          return current;
        }
      });
      UserFiltersState.getPeriod = UserFiltersState.getPeriod.filter(function (
        element
      ) {
  ...
  setStateFromChild: function (
    parentIndex,
    childIndex,
    addIsTrueRemoveIsFalse
  ) {
    if (addIsTrueRemoveIsFalse) {
      // we add to our state
      var newPeriod = UserFiltersState.getPeriod.filter(function (element) {
        if (
          UserFiltersMoreFiltersPeriodSetUp.periodsOptions[parentIndex][
            "options"
          ][childIndex]["instanceDate"] === element["instanceDate"]
        ) {
          return true;
        }
      });
      if (newPeriod.length === 0) {
        UserFiltersState.getPeriod.push(
          UserFiltersMoreFiltersPeriodSetUp.periodsOptions[parentIndex][
            "options"
          ][childIndex]
        );
