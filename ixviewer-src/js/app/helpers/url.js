...
var HelpersUrl = {
  ...
  makeAbsoluteUrlUnlessSimpleAnchorTag: function (element) {
    ...
    if (
      element.getAttribute("href").indexOf("http://") === 0 ||
      ...
      if (element.getAttribute("href").startsWith("#")) {
        ...
        element.setAttribute(
          "href",
          HelpersUrl.getFormAbsoluteURL + element.getAttribute("href")
        );
      }
    }
  },
  ...
  addLinkattributes: function (element) {
    ...
    if (attribute && element.getAttribute(attribute).charAt(0) !== "#") {
      var absoluteLinkOfElementAttribute = decodeURIComponent(
        HelpersUrl.getAbsoluteUrl(element.getAttribute(attribute))
      );
      var url = new ParsedUrl(absoluteLinkOfElementAttribute);
      if (url.search) {
        var urlParams = HelpersUrl.returnURLParamsAsObject(
          url.search.substring(1)
        );
        if (
          urlParams.hasOwnProperty("doc-file") &&
          ...
          element.setAttribute(
            "onclick",
            "Links.clickEventInternal(event, this)"
          );
        } else {
        ...
        if (url.hash) {
          ...
          if (
            element.getAttribute(attribute).split("#")[0] &&
            Constants.getMetaSourceDocuments.indexOf(
              element.getAttribute(attribute).split("#")[0]
            ) >= 0
          ) {
          ...
          var index = Constants.getMetaSourceDocuments.indexOf(
          ...
          if (index >= 0) {
            ...
            element.setAttribute(
              "data-link",
              Constants.getMetaSourceDocuments[index]
            );
            element.setAttribute(
              "href",
              Constants.getMetaSourceDocuments[index]
  ...
  returnURLParamsAsObject: function (url) {
    ...
    var urlSplit = url
      .split(/doc=|file=|metalinks=|xbrl=true|xbrl=false/)
      .filter(function (e) {
        return e;
      });
    ...
    var obj = urlSplit
      .map(function (current) {
        ...
        if (
          current.endsWith(".htm") ||
          current.endsWith(".html") ||
          current.endsWith(".xhtml")
        ) {
    ...
    for (var i = 0; i < obj.length; i++) {
      ...
      if (!single.hasOwnProperty("metalinks")) {
        var metalinks = single["doc"].replace(
          single["doc-file"],
          "MetaLinks.json"
        );
  ...
  setParams: function (internalUrl) {
    if (
      ...
      internalUrl !== HelpersUrl.getHTMLFileName
    ) {
      HelpersUrl.fullURL = HelpersUrl.fullURL.replace(
        HelpersUrl.getHTMLFileName,
        internalUrl
      );
    ...
    var tempUrl = new ParsedUrl(
      url.search.substring(1).replace(/doc=|file=/, "")
    );
    ...
    if (url.search) {
      HelpersUrl.getAllParams = HelpersUrl.returnURLParamsAsObject(
        url.search.substring(1)
      );
      HelpersUrl.getAllParams.hostName = window.location.hostname;

      if (HelpersUrl.getAllParams.hasOwnProperty("metalinks")) {
        HelpersUrl.getExternalMeta = decodeURIComponent(
          HelpersUrl.getAllParams["metalinks"]
        );
        HelpersUrl.getExternalMeta = HelpersUrl.getExternalMeta.replace(
          "interpretedFormat=true",
          "interpretedFormat=false"
        );
      }
      ...
      if (!HelpersUrl.getExternalMeta && HelpersUrl.getExternalFile) {
        var tempMetaLink = HelpersUrl.getExternalFile.replace(
          HelpersUrl.getHTMLFileName,
          "MetaLinks.json"
        );
