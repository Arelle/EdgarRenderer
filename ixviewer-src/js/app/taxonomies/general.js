...
var TaxonomiesGeneral = {
  ...
  addPopover: function (element, isContinued) {
    ...
    var popoversDiscoveredArray =
      Array.prototype.slice.call(popoversDiscovered);

    popoversDiscoveredArray.forEach(function (current) {
      current.parentNode.removeChild(current);
    });

    var terseLabelOnly = FiltersName.getTerseLabelOnlyLabel(
      element.getAttribute("name")
    )
      ? FiltersName.getLabel(element.getAttribute("name"))
      : "Not Available.";
  ...
  removeAllSelectedTaxonomy: function () {
    var otherSelectedTaxonomies = document.querySelectorAll(
      '[selected-taxonomy="true"]'
    );

    var otherSelectedTaxonomiesArray = Array.prototype.slice.call(
      otherSelectedTaxonomies
    );
  ...
  selectedTaxonomy: function (element) {
    ...
    if (element instanceof Array) {
      element.forEach(function (current, index) {
        if (index === 0) {
          var menuTaxonomy = document.querySelector(
            '[data-id="' + current.getAttribute("id") + '"]'
          );
          if (menuTaxonomy) {
      ...
      if (element.getAttribute("id")) {
        foundTaxonomies = document.querySelectorAll(
          "#" +
  ...
  getElementByNameContextref: function (name, contextRef) {
    return document
      ...
      .querySelector('[name="' + name + '"][contextRef="' + contextRef + '"]');
  },
  ...
  goTo: function (event, element, modalPopup) {
    ...
    if (element.getAttribute("data-id")) {
      elementToScrollTo = document
        .getElementById("dynamic-xbrl-form")
        .querySelector("#" + element.getAttribute("data-id"));

      menuElementToScrollTo = document.querySelector(
        '[data-id="' + element.getAttribute("data-id") + '"]'
      );
    } else if (element.getAttribute("id")) {
      elementToScrollTo = document
        .getElementById("dynamic-xbrl-form")
        .querySelector("#" + element.getAttribute("id"));
  ...
  getTaxonomyListTemplate: function (elementID, modalAction) {
    ...
    aElement.addEventListener("click", function (e) {
      TaxonomiesGeneral.goTo(e, aElement, modalAction);
    });
    aElement.addEventListener("keyup", function (e) {
      TaxonomiesGeneral.goTo(e, aElement, modalAction);
    });
    ...
    var pElementContent = document.createTextNode(
      FiltersName.getLabel(element.getAttribute("name")) || ""
    );
    ...
    divElement.appendChild(
      TaxonomiesGeneral.getTaxonomyBadge(element) || document.createTextNode("")
    );
    ...
    if (
      elementContinueAt instanceof Array &&
      elementContinueAt[0].hasAttribute("format")
    ) {
      smallElementContent = document.createTextNode(
        FiltersValue.getFormattedValueForContinuedAt(elementContinueAt, true)
      );
    } else if (
  ...
  getTaxonomyBadge: function (element) {
    ...
    if (
      element.hasAttribute("isAdditionalItemsOnly") &&
      element.getAttribute("isAdditionalItemsOnly") === "true"
    ) {
      label += "A";
      title += "Additional";
    }
    ...
    // custom
    if (
      element.hasAttribute("isCustomOnly") &&
      element.getAttribute("isCustomOnly") === "true"
    ) {
      if (label) {
        label += " & C";
        title += " & Custom";
  ...
  specialSort: function (unsortedArray) {
    ...
    var returnedArray = unsortedArray
