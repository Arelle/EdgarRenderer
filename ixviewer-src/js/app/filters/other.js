...
var FiltersOther = {
    ...
    if ( footnoteID && typeof footnoteID === 'string' ) {
      ...
      idsToGet.forEach(function( current ) {
        ...
        if ( discoveredFootnote ) {
          contentToReturn += discoveredFootnote.textContent;
        }
      });
  ...
  countryNameEn : function( element ) {
    if ( element && typeof element === 'object' && element['textContent'] ) {
      ...
      for ( var option in regexOptions ) {
        if ( regexOptions[option].exec(element['textContent']) && regexOptions[option].exec(element['textContent'])[0] ) {
          return option;
        }
      }
    }
  ...
  entityFilerCategoryEn : function( element ) {
    if ( element && typeof element === 'object' && element['textContent'] ) {
      ...
      for ( var option in regexOptions ) {
        if ( regexOptions[option].exec(element['textContent']) ) {
          return option;
        }
      }
    }
