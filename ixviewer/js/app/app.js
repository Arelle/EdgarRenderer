...
var App = {
	...
	init : function() {
		...
		(function(doc) {
			...
			if (navigator.userAgent.match(/iPhone/i)) {
				...
				document.getElementById("dataFilter").classList
						.add("hidden-md");
				document.getElementById("dataFilter").classList
						.add("hidden-lg");
				...
				document.getElementById("taggedSections").classList
						.add("hidden-md");
				document.getElementById("taggedSections").classList
						.add("hidden-lg");
				document.getElementById("moreFilters").classList
						.add("hidden-md");
				document.getElementById("moreFilters").classList
						.add("hidden-lg");
				document.getElementById("factList").classList.add("hidden-md");
				document.getElementById("factList").classList.add("hidden-lg");
				document.getElementById("taggedSectionsReport").classList
						.remove("hidden-md");
				document.getElementById("taggedSectionsReport").classList
						.remove("hidden-lg");
				document.getElementById("taggedSectionsReport").classList
				...
				document.getElementById("taggedSectionsReport").classList
						.add("visible-lg");
				document.getElementById("dataFilterLink").classList
						.remove("hidden-md");
				document.getElementById("dataFilterLink").classList
						.remove("hidden-lg");
				document.getElementById("dataFilterLink").classList
				...
				document.getElementById("dataFilterLink").classList
						.add("visible-lg");
				document.getElementById("tagsFilterLink").classList
						.remove("hidden-md");
				document.getElementById("tagsFilterLink").classList
						.remove("hidden-lg");
				document.getElementById("tagsFilterLink").classList
		...
		App.showLoadingDialog({
			...
			if (/iPhone|iPod|iPad/.test(navigator.userAgent)) {
				...
				$('#app-panel-logo-container').find('[title]').each(
				...
				$('#selection-detail-container').find('[title]').each(
						function(index, element) {
				...
				$('#app-panel-container').find('[title]').each(
						function(index, element) {
			...
			$('#message-box-container').find('.message-btn').first().on(
					'click',
					function() {
						...
						$('#app-panel-reports-container .toolbar h4').css(
								"marginTop", "5px");
						$('#app-panel-help-container .toolbar h4').css(
								"marginTop", "5px");
	...
	showMessage : function(message, options, callback) {
		...
		if ($("#filterDataDiv").height() == 0) {
			...
			if ($("#message-box-container").height() > 0) {
				$(".modal").css("top", "40px");
				$('#app-panel-reports-container .toolbar h4').css("marginTop",
						"40px");
				$('#app-panel-help-container .toolbar h4').css("marginTop",
						"40px");
	...
	showLoadingDialog : function(options, callback) {
		...
		if (options.message) {
			...
			$('.progress-container').find('h3').text(options.message).css(
					'display', 'block');
		} else {
		...
		setTimeout(
				function() {
					try {
						...
						App
								.showMessage("Selected feature has encountered a processing issue.");
						App.hideSpinner();
					}
				}, 200);
	},
	...
	updateLoadingDialogProgress : function(percent, callback) {
		...
		$('.progress-container').find('.progress-bar').css('width',
				percent + '%');
	...
	updateDocStyle : function() {
		...
		var styleStr = '.sec-cbe-highlight-dashed { border-top: 2px solid '
				+ App_Settings.get('elementBorderColor')
				+ '!important; cursor: pointer; border-bottom: 2px solid '
				+ App_Settings.get('elementBorderColor')
				...
				+ '.sec-cbe-highlight-dashed_block { border-left: 0px solid '
				+ App_Settings.get('elementBorderColor')
				...
				+ '.sec-cbe-highlight-hover-content {background-color:'
				+ App_Settings.get('blockHighlightColor')
				+ ' !important; cursor: pointer;border: 2px dotted '
				+ App_Settings.get('elementBorderColor')
				+ '}'
				+ '.sec-cbe-highlight-hover-over-content-selected {border: 2px solid '
				+ App_Settings.get('focusContentSelectionColor')
				+ '!important; cursor: pointer;}'
				+ '.sec-cbe-highlight-content-selected { background-color:'
				+ App_Settings.get('blockHighlightColor')
				+ ' !important ; cursor: pointer;}'
				+ '.sec-cbe-highlight-filter-block-content-selected { background-color:'
				+ App_Settings.get('blockHighlightColor')
				+ ' !important ; cursor: pointer;}'
				+ '.sec-cbe-highlight-left-border {border-left: 2px dashed '
				+ App_Settings.get('focusHighlightColor')
				+ ';}'
				+ '.sec-cbe-highlight-filter { background-color:'
				+ App_Settings.get('initialHighlightColor')
				+ '!important; }'
				+ '.sec-cbe-highlight-filter * { background-color:'
				+ App_Settings.get('initialHighlightColor')
				+ '!important; }'
				+ '.sec-cbe-highlight-filter-selected { border: 3px solid '
				+ App_Settings.get('focusHighlightColor')
				+ '!important; cursor: pointer; }'
				...
				+ App_Settings.get('blockHighlightColor')
				+ '  ;cursor: pointer; }'
				+ '.sec-cbe-highlight-filter-selected-nodes { border: 0px solid '
				+ App_Settings.get('focusHighlightColor')
				+ '!important; cursor: pointer; }'
				+ '.sec-cbe-highlight-filter-content-selected {background-color:'
				+ App_Settings.get('blockHighlightColor')
				+ '; cursor: pointer;}' +
		...
		if (style.length == 0) {
			...
			frameHead.append('<style id="sec-cbe-style" type="text/css">'
					+ styleStr + '</style>');
		} else {
...
var App_RemoteDocs = {
	init : function() {
		...
		if (regexReturnsFalseIfRelativeUrl.test(query)) {
			{
				if (query.startsWith(window.location.origin)) {
					...
					$('.toolbarSpinner').remove();
					App
							.showMessage('The protocol, host name and port number of the \'doc\' field ('
									+ query
									+ '), if provided, must be identical to that of the Inline XBRL viewer('
		...
		if (docPath && docPath != '') {
			...
			App.fileNames.filing.inline = filename.substring(0, filename
					.indexOf('.'));
	...
	initSaveAs : function(docPath, docBasePath) {
		...
		var fileName = docBasePath.substring(0, docBasePath.indexOf('.'))
				+ "_htm.xml";
		var openfileName = docBasePath.substring(0, docBasePath.indexOf('.'))
				+ ".htm";
		...
		if (lastFolder.length == 18 && !(isNaN(lastFolder))) {
			...
			zipFileName = lastFolder.replace(/(\d{10})(\d{2})(\d{6})/,
					"$1-$2-$3")
					+ "-xbrl.zip";
		} else {

			zipFileName = docBasePath.substring(0, docBasePath.indexOf('.'))
					+ ".zip";
		}
			...
			$('#openAsHtml')
					.on(
							...
							function() {
								...
								App
										.showMessage("The Inline XBRL document could not be found.");
							});
		}
	},
		...
		if (App.InlineDoc.nsLookupByKey(App.InlineDoc.standardTaxonomy)) {
			...
			if (schemaFilename) {
				...
				var setXBRLDoc = function() {
					...
					if (docsLoadCompleted.schema && docsLoadCompleted.label
							&& docsLoadCompleted.calculation) {
				...
				App_Utils.loadDocument(docBasePath + "/" + schemaFilename,
						function(schemaDoc) {
				...
				App_Utils.loadDocument(docBasePath + "/" + labelFilename,
						function(labelDoc) {
				...
				App_Utils.loadDocument(docBasePath + "/" + calcFilename,
						function(calcDoc) {
...
var App_XBRL = {
	saveFilingToDisk : function() {
		...
		var filename = App.fileNames.filing.inline == ''
				? 'filing'
				: App.fileNames.filing.inline;
		var xmlFilename = App.fileNames.filing.inline == ''
				? 'inline'
				: App.fileNames.filing.inline;
		var zip = new JSZip();
	...
	inlineDocToXML : function() {
		...
		var p = App.InlineDoc.nsLookupByValue(App.InlineDoc.namespaces.xbrli,
				true);
		var xmlStr = '<?xml version="1.0" encoding="utf-8"?>'
				+ '<!-- Generated By ViewFinder on '
				+ (new Date()).toDateString() + ' -->' + '<' + p + ':xbrl '
				+ namespaceStr + ' xmlns="http://www.w3.org/1999/xhtml">';

		// references
		var references = xmlDoc.find(App.InlineDoc.inlinePrefix
				+ '\\:references, references');
		xmlStr += references.html();

		// resources
		var resources = xmlDoc.find(App.InlineDoc.inlinePrefix
				+ '\\:resources, resources');
		...
		elements
				.each(function(index, element) {
					...
					if ($.inArray(node.attr('name'), processedElements) == -1
							&& (node.attr('name') && element.nodeName
									.toLowerCase().split(':')[0] == App.InlineDoc.inlinePrefix)) {
						...
						if (!xbrlValue) {
							...
							if (node.attr('format')) {
								...
								xbrlValue = ixtTransform(node.attr('format'),
										node.text());
							} else {
								...
								htmlInstance
										...
										.each(
												function(index, element) {
													...
													if (element.nodeName
															.toLowerCase()
															.split(':')[0] == App.InlineDoc.inlinePrefix) {

														$(element).contents()
																.unwrap();
													}
												});
								xbrlValue = htmlEnDeCode
										.htmlEncode(htmlInstance.html());
							}
						} else {
							...
							if (xbrlValue == 'nil') {
								...
								xbrlValue = xbrlValue.toString().replace(
										/[^0-9\.]+/g, '');
						...
						var attributeStr = '';
						var excludedAttributes = ["name", "scale", "format",
								"sign", "escape"];
						...
						// create the tag
						xmlStr += '<' + node.attr('name') + ' ' + attributeStr
								+ '>' + xbrlValue + '</' + node.attr('name')
								+ '>';
					}
				});
...
$(window)
		.load(
				function() {
					/*
					 * Due to the iframe functionality being used We need to
					 * accomplish a 'sanity check' First we check the HEAD of
					 * the document the user has requested If that is
					 * successful, we continue, Otherwise we throw an error to
					 * the user, saying we can not find The requested file
					 */
										...
										.ajax({
											...
											success : function(requestObject) {
												...
												App.frame
														.attr(
																'src',
																ApplicationObject.urlDocumentPath);
												App.frame = $('#app-inline-xbrl-doc');
												App.frame
														.on(
																'load',
																function(event) {
																	...
																	// so we can
																	// query it
																	App.InlineDoc = new cbe.InlineDoc(
																			App.frame);
					...
					if (($.browser.msie && $.browser.versionNumber >= 10)
						...
						if ((null != navigator.userAgent.match('CriOS'))
						...
						if (userAgent.indexOf('ipad') != -1
							...
							note += ' <br>Please use a more current version of '
									+ browserName
...
function ixtTransform(ns, localName, text) {
	...
	if (!ixtFunctions) {
		...
		if (!ixtFunction) {
			return localName + " transformation not implemented in namespace "
					+ ns;
		} else {
...
var sakaMonthOffset = [[3, 22, 0], [4, 21, 0], [5, 22, 0], [6, 22, 0],
...
function jpDigitsToNormal(jpDigits) {
	...
	for (var i = 0; i < jpDigits.length; i++) {
		var d = jpDigits[i];
		if ('\uFF10' <= d && d <= '\uFF19')
			normal += String.fromCharCode(d.charCodeAt(0) - 0xFF10
					+ '0'.charCodeAt(0));
		else
			normal += d;
	}
...
function sakaToGregorian(sYr, sMo, sDay) {
	...
	// yr
	if (gYr < 0)
		return "Saka calendar year not supported: " + sYr + " " + sMo + " "
				+ sDay;
	if (sMo < 1 || sMo > 12)
...
function countrynameen(arg) {
	...
	var getCountryCode = {
		...
		'AX' : ['Åland Islands', 'Åland Islands', 'Åland Islands',
				'Aland Islands', 'aland islands', 'aland Islands'],
		'AL' : ['Albania'],
		'DZ' : ['Algeria'],
		'AS' : ['American Samoa'],
		'AD' : ['Andorra'],
		...
		'BW' : ['Botswana'],
		'BV' : ['Bouvet Island'],
		'BR' : ['Brazil', 'Brasil', 'The Federative Republic of Brazil',
		...
		'US' : ['United States of America', 'United States', 'USA', 'U.S.A.'],
		'UY' : ['Uruguay'],
		'UZ' : ['Uzbekistan'],
		'VU' : ['Vanuatu'],
		'VE' : ['Venezuela'],
	...
	var argRegex = new RegExp(arg.trim(), '');
	for ( var i in getCountryCode) {
		for ( var k in getCountryCode[i]) {
			var result = argRegex.exec(getCountryCode[i][k]);
			if (result
					&& (result.index === 0 && result.input.length === result[0].length)) {
				return i;
			}
		}
	}
...
function edgarprovcountryen(arg) {
	var getEdgarProvCountryEn = {
		...
		'A9' : ['Saskatchewan'],
		'B0' : ['Yukon'],
		'B2' : ['Afghanistan', 'afghanistan', ' Afghanistan ', ' afghanistan '],
		...
		'X1' : ['United States of America', 'United States', 'USA', 'U.S.A.'],
		'X3' : ['Uruguay'],
		'2K' : ['Uzbekistan'],
		'2L' : ['Vanuatu'],
		'X5' : ['Venezuela'],
	...
	var argRegex = new RegExp(arg, '');

	for ( var i in getEdgarProvCountryEn) {
		for ( var k in getEdgarProvCountryEn[i]) {
			var result = argRegex.exec(getEdgarProvCountryEn[i][k]);
			if (result
					&& (result.index === 0 && result.input.length === result[0].length)) {
				return i;
			}
...
function stateprovnameen(arg) {
	...
	for ( var i in getStateProvidenceNames) {
		for ( var k in getStateProvidenceNames[i]) {
			var result = argRegex.exec(getStateProvidenceNames[i][k]);
			if (result
					&& (result.index === 0 && result.input.length === result[0].length)) {
				return i;
			}
...
function exchnameen(arg) {
	...
	var getExchangeCodes = {
		'BOX' : ['BOX Exchange LLC', 'box exchange llc', 'BOX Exchange'],
		'CboeBYX' : ['Cboe BYX Exchange, Inc.', 'Cboe BYX Exchange, Inc',
				'Cboe BYX Exchange', 'Cboe byx Exchange, Inc.'],
		'CboeBZX' : ['Cboe BZX Exchange Inc.'],
		'C2' : ['Cboe C2 Exchange, Inc.'],
		'CboeEDGA' : ['Cboe EDGA Exchange, Inc.', 'Cboe edga Exchange, Inc.'],
		...
		'NYSE' : ['New York Stock Exchange', 'New York Stock Exchange LLC',
	...
	for ( var i in getExchangeCodes) {
		for ( var k in getExchangeCodes[i]) {
			var result = argRegex.exec(getExchangeCodes[i][k]);
			if (result
					&& (result.index === 0 && result.input.length === result[0].length)) {
				return i;
			}
...
function entityfilercategoryen(arg) {
	...
	for ( var i in getEntityFilerCategory) {
		for ( var k in getEntityFilerCategory[i]) {
			var result = argRegex.exec(getEntityFilerCategory[i][k]);
			if (result
					&& (result.index === 0 && result.input.length === result[0].length)) {
				return i;
			}
...
function datedaymonthdk(arg) {
	...
	if (m) {
		...
		if (mon3 in monthnumber) {
			...
			if (((!monEnd && !monPer) || (!monEnd && monPer) || (monEnd && !monPer))
...
function datedaymonthyear(arg) {
	...
	if (result) {
		...
		var dateResult = moment(arg, ['DD MM YY', 'DD.MM.YYYY', 'DD.MM.Y',
				'DD.MM.YY', 'D.M.YY', 'DD/MM/YY'], true);
...
function datemonthyearin(arg) {
	...
	if (m[1] in gregorianHindiMonthNumber)
		return yr(devanagariDigitsToNormal(m[2])) + '-'
				+ gregorianHindiMonthNumber[m[1]];
	return "ixt:dateError";
}
...
function datedaymonthyeardk(arg) {
	...
	if (result) {
		...
		if (month && ((!monthEnd && !monthPer) || (!monthEnd && monthPer))
				|| (monthEnd && !monthPer)) {
...
function calindaymonthyear(arg) {
	...
	return gregorianDate[0] + "-" + z2(gregorianDate[1]) + "-"
			+ z2(gregorianDate[2]);
}
...
function dateyearmonthday(arg) {
	...
	if (result) {
		...
		var dateResult = moment(year + '-' + month + '-' + day, ['YYYY-MM-DD',
				'YYYY-MM-D', 'YYYY-M-DD', 'YY-M-DD', 'Y-M-DD'], true);
...
function numdotdecimalin(arg) {
	...
	if (result) {
		...
		return arrayResult[0].replace(/\,/g, '').replace(/ /g, '').replace(
				/\xa0/g, '')
				+ (fraction ? fraction : '');
	}
...
for (k in tr2Functions)
	tr3Functions[k] = tr2Functions[k];

deprecatedNamespaceURI = 'http://www.xbrl.org/2008/inlineXBRL/transformation' // the
// CR/PR
// pre-REC
// namespace
...
numwordsNoPattern = /^\s*([Nn]o(ne)?|[Nn]il)\s*$/;
...
function numwordsen(arg) {
	...
	if (regex.exec(arg)) {
		...
		if (arg.length > 0 && result) {
			...
			var returnString = arg.trim().toLowerCase()
					.replace(thirdRegex, " ");
		...
		for (var i = 0; i < grp.length; i++) {
			...
			if (groupPart != null) {
				if (durwordZeroNoPattern.exec(groupPart) == null) {
					if (isNaN(groupPart)) {
						var tmp = groupPart.trim().toLowerCase().replace(
								commaAndPattern, " ");
						dur += text2num(tmp);
					} else {
						dur += groupPart;
...
var App_About = {
	...
	carouselGoTo : function(index) {
		...
		if (index >= 0 && index < App_About.carouselItems.length) {
			...
			var container = $('#about-modal');
			container.find('span[data-content="subtitle"]').text(
	...
	load1 : function() {
		...
		if (App_About.companyInformation == null) {
			...
			modal1.find('span[data-content="companyName"]').text(
					App_About.companyInformation.name);
			modal1.find('span[data-content="companyCIK"]').text(
					App_About.companyInformation.cik);
			modal1.find('span[data-content="companyDocument"]').text(
					App_About.companyInformation.document);
			modal1.find('span[data-content="companyPeriodEndDate"]').text(
					App_About.companyInformation.periodEndDate);
			modal1.find('span[data-content="companyFiscalYearAndPeriodFocus"]')
					.text(
							App_About.companyInformation.fiscalYearFocus
									+ " / "
									+ App_About.companyInformation.periodFocus);
			modal1.find('span[data-content="companyFiscalYear"]').text(
					App_About.companyInformation.fiscalYear);
			modal1.find('span[data-content="companyAmendment"]').text(
					App_About.companyInformation.amendment);
			...
			if (App_About.companyInformation.amendment == 'true') {
				...
				if (amendmentDescription.length > maxValueTextLength) {
					...
					amendmentDescription = amendmentDescription.trim()
							.substring(0, maxValueTextLength)
							+ '...';
				}

				amendmentDescription = ' / ' + amendmentDescription;
			}
			modal1.find('span[data-content="companyAmendmentDescription"]')
					.text(amendmentDescription);
		...
		for (var i = 0; i < sections.length; i++) {
			...
			if (row) {
				...
				if (sectionName == 'total') {
					...
					var totalStandard = facts.key_concepts.standard
							+ facts.axis.standard + facts.member.standard;
					var totalCustom = facts.key_concepts.custom
							+ facts.axis.custom + facts.member.custom;
					...
					if (totalStandard > 0) {
						$(tds[1]).text(
								Math.round((totalStandard / totalAll) * 100)
										+ '%');
					} else {
						$(tds[1]).text('0%');
					}
					if (totalCustom > 0) {
						$(tds[3]).text(
								Math.round((totalCustom / totalAll) * 100)
										+ '%');
					} else {
						$(tds[3]).text('0%');
					}
					...
					var total = facts[sectionName].standard
							+ facts[sectionName].custom;
					...
					if (facts[sectionName].standard > 0) {
						$(tds[1])
								.text(
										Math
												.round((facts[sectionName].standard / total) * 100)
												+ '%');
					} else {
						$(tds[1]).text('0%');
					}
					if (facts[sectionName].custom > 0) {
						$(tds[3])
								.text(
										Math
												.round((facts[sectionName].custom / total) * 100)
												+ '%');
					} else {
						$(tds[3]).text('0%');
					}
		...
		for ( var i in nameList) {
			for ( var doctype in nameList[i]) {
				...
				if (dts) {
					...
					fileNamesHTML += '<tr><td>' + docpretty + '</td><td>'
							+ "N/A" + '</td></tr>';
				}
			}
			if (i == 0) {
	...
	additionalItems : function() {
		...
		hiddenItemTable.append($('<thead>').append(
				$('<tr>').append($('<th>').text('Taxonomy'),
						$('<th>').text('Count').css('text-align', 'center'))));
		var tbody = $('<tbody>');
		hiddenItemTable.append(tbody);
		for ( var index in hiddenElements) {
			if (index != 'total') {
				tbody.append($('<tr>').append(
						$('<td>').text(index).attr('width', '70%').css(
								'max-width', '260px').css('word-wrap',
								'break-word')).append(
						$('<td>').text(hiddenElements[index]).attr('width',
								'70%').css('text-align', 'right')));
			}
		}
		tbody.append($('<tr>').append(
				$('<th>').text('Total (Across all source documents)').attr(
						'width', '70%').css('max-width', '260px').css(
						'word-wrap', 'break-word')).append(
				$('<th>').text(hiddenElements['total']).attr('width', '70%')
	...
	copyToClipboard : function() {
		...
		container.find('div[data-slide-index="0"] tr').each(
				function(index, element) {
					var x = 0;
		...
		for (var i = 0; i < tableID.rows.length; i++) {
			for (var j = 0; j < tableID.rows[i].cells.length; j++) {
				...
				clipboardText += element.replace(/\s+/g, " ").replace(
						/^\s|\s$/g, "");
				clipboardText += "&nbsp;&nbsp;&nbsp;";
			}
			clipboardText += "\n";
		...
		clipboardText += "\nAdditional Items";
		$('#about-modal').find('div[data-slide-index="3"]').find('tr').each(
				function(index, element) {
					var x = 0;
					$(this).find('td').each(function(index, element) {
						var node = $(element);
		...
		if ($.browser.msie) {
			myWindow.document
					.write("<html><head><title>Copy All</title></head>"
							+ "<body><textarea id='popTxtArea' rows='25' cols='50' style='width:98.7%'>"
							+ clipboardText + "</textarea></body></html>");
		} else {
			myWindow.document
					.write("<html><head><title>Copy All</title></head>"
							+ "<body><textarea id='popTxtArea' rows='25' cols='50'>"
							+ clipboardText + "</textarea></body></html>");
		}
				...
				.on(
						...
						function() {
							...
							if (screen.width < 641) {
								...
								$('#app-panel1').removeClass('visible')
										.animate({
											'margin-left' : '-100%'
										});
...
var App_Settings = {
	...
	init : function() {
		...
		var colorOptions = {
			...
			onSelect : function(color, input) {
				...
				if (input.prop('id') == 'setting-initial-highlight-color') {
					...
					App_Settings.save('focusHighlightSelectionColorCode', color
							.toLowerCase());
					...
					App_Settings.save('elementBorderColorCode', color
							.toLowerCase());
					window.location.reload();
				}
			}
		...
		App_Settings.initialHighlightColor.val(
				App_Settings.get('initialHighlightColor')).simpleColor(
				colorOptions);
		App_Settings.focusHighlightColor = $('#setting-focus-highlight-color');
		App_Settings.focusHighlightColor.val(
				App_Settings.get('focusHighlightColor')).simpleColor(
				colorOptions);
		App_Settings.elementBorderColor = $('#setting-element-border-color');
		App_Settings.elementBorderColor.val(
				App_Settings.get('elementBorderColor')).simpleColor(
				colorOptions);
		App_Settings.blockHighlightColor = $('#setting-block-highlight-color');
		App_Settings.blockHighlightColor.val(
				App_Settings.get('blockHighlightColor')).simpleColor(
				colorOptions);
	},
	get : function(key) {
