...
var App_XBRL = {
	...
	var filename = App.fileNames.filing.inline == '' ? 'filing'
		: App.fileNames.filing.inline;
	var xmlFilename = App.fileNames.filing.inline == '' ? 'inline'
		: App.fileNames.filing.inline;
	var zip = new JSZip();
...
function datedaymonthyeardk(arg) {
	...
	if (_mon3 in monthnumber
		&& ((!_monEnd && !_monPer) || (!_monEnd && _monPer) || (_monEnd && !_monPer))) {
...
function datedaymonthyearin(arg) {
	...
	var _mo = gregorianHindiMonthNumber.get(m[2],
		devanagariDigitsToNormal(m[2]));
...
function dateyearmonthday(arg) {
    var m = yearmonthdayPattern.exec(jpDigitsToNormal(arg)); // (Y)Y(YY)*MM*DD
    // with kangu
    // full-width
    // numerals
    if (m) {
...
function numdotdecimalin(arg) {
	...
	if (lastm && lastm.charAt(0) == ".")
	    ...
	    fract = "";
	return m2[0].replace(',', '').replace(' ', '').replace('\xa0', '')
		+ fract;
    }
    return "ixt:numberPatternError";
}
...
var App_About = {
	...
	tbody.append($('<tr>').append(
		$('<th>').text('Total').attr('width', '70%').css('max-width',
			'260px').css('word-wrap', 'break-word')).append(
