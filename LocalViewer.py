...
def init(cntlr, reportsFolder): # returns browser root
    ...
    try:
            ...
            def getlocalfile(file=None, relpath=None):
                ...
                try:
                    ...
                    if _report.isnumeric(): # in reportsFolder folder
                        ...
                        _fileDir = reportsFolders[int(_report)]
                        _fileExists = False
                        if os.path.exists(os.path.join(_fileDir, _file)):
                            ...
                            if os.path.exists(os.path.join(os.path.dirname(_fileDir), _file)):
                                ...
                                _fileExists = True
                        if not _fileExists:
                            cntlr.addToLog("http://localhost:{}/{}".format(port,file), messageCode="localViewer:fileNotFound",level=logging.DEBUG)
                    ...
                    cntlr.addToLog(_("SEC ix local viewer exception: file: {} \nException: {} \nTraceback: {}").format(
        ...
        cntlr.addToLog(_("SEC ix viewer: http://localhost:{}").format(port), messageCode="localViewer:listen",level=logging.DEBUG)
        #cntlr.addToLog("localhost={}".format(localhost), messageCode="localViewer:listen",level=logging.DEBUG)
        return localhost
    except Exception as ex:
        cntlr.addToLog(_("SEC ix local viewer exception: http://localhost:{} \nException: {} \nTraceback: {}").format(
