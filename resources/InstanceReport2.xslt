...
<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform">
  ...
  <xsl:template name="authRefLink">
    <!-- [<xsl:value-of select="../../../IsTransposed" />] -->
        ...
        <xsl:choose>
          ...
          <xsl:when test="IsNumeric = 'true'">
            <xsl:choose>
              ...
              <xsl:otherwise>
                <xsl:attribute name="class">num<xsl:if test="NumericAmount &gt;= 0">p</xsl:if>
                  <xsl:if test="$showFlags = 'true' and @FlagID > 0">Flag<xsl:value-of select="@FlagID"/></xsl:if>
                </xsl:attribute>
  ...
  <xsl:template name="numFilters">
    ...
    <xsl:choose>
      ...
      <xsl:otherwise>
        ...
        <xsl:choose>
          <xsl:when test="string-length( $decimal ) = 0">
            <xsl:value-of select="format-number( $whole, 'D,DDD,DDD,DDD,DDD,DDD,DDD,DD0', 'currency')"/>
          </xsl:when>
          <xsl:otherwise>
            <xsl:value-of select="format-number( $whole, 'D,DDD,DDD,DDD,DDD,DDD,DDD,DD0', 'currency')"/>.<xsl:value-of select="$decimal"/>
          </xsl:otherwise>
        </xsl:choose>
      </xsl:otherwise>
    </xsl:choose>
