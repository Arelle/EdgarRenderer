...
<xsl:template match="/">
	...
	<xsl:choose>
		<xsl:when test="$asPage">
			<html
			  ...
			  xsi:schemaLocation="http://www.w3.org/1999/xhtml http://www.xbrl.org/2013/inlineXBRL/xhtml-inlinexbrl-1_1.xsd"
			  xmlns:ix="http://www.xbrl.org/2013/inlineXBRL" xmlns:ixt="http://www.xbrl.org/inlineXBRL/transformation/2011-07-31"
			  >
			<head>
			  <title><xsl:value-of select="InstanceReport/ReportLongName"/></title>
    ...
    <xsl:template name="header">
		...
		<th class="th">
			...
			<xsl:otherwise>
				...
				<xsl:value-of select="@Label"/><xsl:element name="br"/>
				</xsl:for-each>
	...
	<xsl:template name="dateFilters">
	  ...
	  <xsl:choose>
	  <xsl:when test="string-length(@ContextID) > 0">
	    <xsl:element xmlns:ix="http://www.xbrl.org/2013/inlineXBRL" 	      
	      name="ix:nonNumeric">
	      <xsl:variable name="en" select="../../ElementName"/>
	      <xsl:variable name="ep" select="../../ElementPrefix"/>
	      <xsl:variable name="nsprefix" select="substring($ep,1,string-length($ep)-1)"/>
	      <xsl:attribute name="name">
	        <xsl:value-of  select="concat($nsprefix,':',substring($en,string-length($ep)+1))"/>
	      </xsl:attribute>
	      <xsl:attribute name="contextRef"><xsl:value-of select="@ContextID"/></xsl:attribute>
	      <xsl:attribute name="id"><xsl:value-of select="generate-id()"/></xsl:attribute>
	      <xsl:attribute name="format">ixt:ixt:datemonthdayyearen</xsl:attribute>
    ...
    <xsl:if test="$negative">(</xsl:if>
      ...
      <xsl:element xmlns:ix="http://www.xbrl.org/2013/inlineXBRL" 
        ...
        <xsl:attribute name="contextRef"><xsl:value-of select="@ContextID"/></xsl:attribute>
        <xsl:attribute name="unitRef"><xsl:value-of select="@UnitID"/></xsl:attribute>
        <xsl:attribute name="id"><xsl:value-of select="generate-id()"/></xsl:attribute>
        <xsl:variable name="quotient" select="round(NumericAmount div RoundedNumericAmount)"/>
        <xsl:attribute name="scale"><xsl:value-of select="string-length($quotient)-1"/></xsl:attribute>
        <xsl:attribute name="decimals"><xsl:value-of select="1-string-length($quotient)"/></xsl:attribute>
        <xsl:attribute name="format">ixt:numcommadot</xsl:attribute>
        <xsl:if test="0 > NumericAmount"><xsl:attribute name="sign">-</xsl:attribute></xsl:if>
      ...
      <xsl:choose>
        <xsl:when test="string-length(@ContextID) > 0">
          <xsl:element xmlns:ix="http://www.xbrl.org/2013/inlineXBRL" 
            ...
            <xsl:attribute name="contextRef"><xsl:value-of select="@ContextID"/></xsl:attribute>
            <xsl:attribute name="id"><xsl:value-of select="generate-id()"/></xsl:attribute>
            <xsl:if test="contains(NonNumbericText,'&amp;')">
              <xsl:attribute name="escape">true</xsl:attribute>
            </xsl:if>            
    ...
    <xsl:template name="innerFootnotes">
		...
		<xsl:if test="count( Footnotes/Footnote ) &gt; 0">
			...
			<tr class="innerFootnote">
				<td style="vertical-align:top;">[<xsl:value-of select="NoteId" />]</td>
				<td colspan="{ $cols + $idxs }" style="vertical-align:top;"><xsl:value-of disable-output-escaping="yes" select="Note" /></td>
			</tr>
			</xsl:for-each>
		</xsl:if>
    </xsl:template>
	...
	<xsl:template name="outerFootnotes">
		...
		<table class="outerFootnotes" width="100%">
			...
			<tr class="outerFootnote">
				<td style="vertical-align:top;">[<xsl:value-of select="NoteId" />]</td>
				<td style="vertical-align:top;"><xsl:value-of disable-output-escaping="yes" select="Note" /></td>
			</tr>
			</xsl:for-each>
		</table>
		</xsl:if>
