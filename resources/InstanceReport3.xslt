...
<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform">
  ...
  <xsl:param name="showCellMetadata">false</xsl:param>
          ...
          <xsl:when test="IsNumeric = 'true'">
            <xsl:choose>
              ...
              <xsl:otherwise>
                ...
                <xsl:choose>
                  <xsl:when test="$showCellMetadata = 'true'">
                    <xsl:variable name="tmp">
                      <xsl:call-template name="cellMetadata"/>
                    </xsl:variable>
                    <a>
                    ...
                    <xsl:if test="string-length($tmp) > 0">
                      <span style="display:none;white-space:normal;text-align:left;">
                        <xsl:call-template name="cellMetadata">
                          <xsl:with-param name="breaks">true</xsl:with-param>
                        </xsl:call-template>
                      </span>
                    </xsl:if>
                  </xsl:when>
  ...
  <xsl:template name="cellMetadata">
    <xsl:param name="ColumnId" select="Id"/>
    <xsl:param name="breaks">false</xsl:param>
    <xsl:choose>
      <xsl:when test="not($showCellMetadata = 'true')"/>
      <xsl:otherwise>
        <xsl:value-of select="ancestor::Row[1]/ElementName"/>
        <xsl:for-each select="((ancestor::InstanceReport[1]/Columns/Column[Id = $ColumnId] 
                              | ancestor::Row[1])/MCU/contextRef/Segments/Segment/DimensionInfo)">
          <xsl:sort data-type="text" select="substring-after(dimensionId, '_')"/>
          <xsl:choose>
            <xsl:when test="$breaks = 'true'">
              <br/>
              <xsl:text>/ </xsl:text>
